...
function loadBrowsers($name, $mobile) {
  ...
  $result = mysql_queryf("SELECT useragents.engine as engine, useragents.name as name, (SELECT COUNT(*) FROM clients WHERE useragent_id=useragents.id AND updated > DATE_SUB(NOW(), INTERVAL 1 minute)) as clients, (engine=%s AND %s REGEXP version) as found, IFNULL(useragents.icon, useragents.engine) as icon FROM useragents WHERE active=1 AND mobile=%s ORDER BY engine, name;", $browser, $version, $mobile);
  ...
  while ( $row = mysql_fetch_array($result) ) {
    ...
    ?>
		<div class="browser<?php echo $row[0] != $engine ? ' clear' : '';?><?php echo $row[3] ? ' you' : '';?>">
			<img src="<?php echo swarmpath( "images/{$row[4]}.sm.png" ); ?>" class="browser-icon <?php echo $row[4]; ?>" alt="<?php echo $row[1]; ?>" title="<?php echo $row[1]; ?>"/>
